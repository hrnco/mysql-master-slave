...
mkdir -p storage/mysql_master
mkdir -p storage/mysql_slave
...
MASTER_LOG_FILE=$( echo $MASTER_STATUS | awk -F' ' '{print $2}')
MASTER_LOG_POS=$( echo $MASTER_STATUS | awk -F' ' '{print $4}')
